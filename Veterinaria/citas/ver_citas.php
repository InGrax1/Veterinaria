<?php
// 1. Definir variables para la plantilla
$page_title = "Mis Citas - Patitas Felices";
$page_css = "/Veterinaria/Veterinaria/css/ver_citas.css"; // Ruta absoluta al CSS

// 2. Incluir el header (subiendo un nivel con ../)
// La plantilla ya inicia la sesión y la comprueba
include '../includes/plantilla_header.php';

// 3. Lógica específica de esta página
include '../includes/scriptdb.php';

// La validación de sesión ya no es necesaria aquí
// if (!isset($_SESSION['id_propietario'])) { ... }

$id_propietario = $_SESSION['id_propietario'];

// ---- NUEVA LÓGICA DE FILTRADO ----
// 1. Obtenemos el filtro de la URL, por defecto será 'proximas'
$filtro = isset($_GET['filtro']) ? $_GET['filtro'] : 'proximas';

// 2. Preparamos el query base
$sql = "SELECT c.id_cita, c.fecha_hora, c.motivo, m.nombre AS nombre_mascota, c.estado_atencion
        FROM cita_medica c
        INNER JOIN mascota m ON c.id_mascota = m.id_mascota
        WHERE m.id_propietario = ?";

// 3. Añadimos la condición del filtro
$filtro_sql = "";
$order_sql = " ORDER BY c.fecha_hora ASC"; // Por defecto, las próximas primero

if ($filtro == 'pasadas') {
    $filtro_sql = " AND c.fecha_hora < NOW()";
    $order_sql = " ORDER BY c.fecha_hora DESC"; // Las pasadas, de la más reciente a la más antigua
} elseif ($filtro == 'proximas') {
    $filtro_sql = " AND c.fecha_hora >= NOW()";
}
// Si $filtro == 'todos', no se añade filtro de fecha y se usa el $order_sql por defecto

// 4. Combinamos el query
$sql = $sql . $filtro_sql . $order_sql;
// ---- FIN DE LÓGICA DE FILTRADO ----

$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $id_propietario);
$stmt->execute();
$result = $stmt->get_result();
?>

<main class="page-content">
    <div class="table-container">
        <h2>Mis Citas Médicas</h2>

        <div class="filter-buttons">
            <a href="ver_citas.php?filtro=proximas" class="btn-filter <?php if ($filtro == 'proximas') echo 'active'; ?>">Próximas</a>
            <a href="ver_citas.php?filtro=pasadas" class="btn-filter <?php if ($filtro == 'pasadas') echo 'active'; ?>">Pasadas</a>
            <a href="ver_citas.php?filtro=todos" class="btn-filter <?php if ($filtro == 'todos') echo 'active'; ?>">Todas</a>
        </div>

        <?php if ($result->num_rows > 0): ?>
            <div class="table-wrapper">
                <table class="citas-table">
                    <thead>
                        <tr>
                            <th>Fecha y Hora</th>
                            <th>Mascota</th>
                            <th>Motivo</th>
                            <th>Estado</th>
                            <th>Acción</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                <td><?= htmlspecialchars($row['fecha_hora']) ?></td>
                                <td><?= htmlspecialchars($row['nombre_mascota']) ?></td>
                                <td><?= htmlspecialchars($row['motivo'] ?: 'Sin motivo') ?></td>
                                <td>
                                    <?php 
                                    $estado = $row['estado_atencion'];
                                    $clase_estado = 'estado-' . htmlspecialchars($estado);
                                    echo '<span class="estado-cita ' . $clase_estado . '">' . ucfirst($estado) . '</span>';
                                    ?>
                                </td>
                                <td>
                                    <?php if ($estado != 'atendida'): ?>
                                        <form method="POST" action="cancelar_cita.php" onsubmit="return confirm('¿Estás seguro de cancelar esta cita?');">
                                            <input type="hidden" name="id_cita" value="<?= $row['id_cita'] ?>">
                                            <button type="submit" class="btn-cancelar">Cancelar</button>
                                        </form>
                                    <?php else: ?>
                                        <p class="ya-atendida">Cita Finalizada</p>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
            </div>
        <?php else: ?>
            <div class="no-citas-container">
                <img src="data:image/jpeg;base64," alt="Gato durmiendo" class="no-citas-img">
                <h3>No tienes citas programadas</h3>
                <p>Parece que tus mascotas estan al dia con sus cuidados, Excelente trabajo!</p>
                <a href="regicita.php" class="btn btn-primary">Agendar una Cita</a>
            </div>
        <?php endif; ?>

    </div>
</main>

<?php
// 5. Incluir el footer (subiendo un nivel con ../)
include '../includes/plantilla_footer.php';

// 6. Cerrar conexiones
$stmt->close();
$conn->close();
?>