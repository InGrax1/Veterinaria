...
// 1. Obtenemos el filtro de la URL, por defecto será 'proximas'
$filtro = isset($_GET['filtro']) ? $_GET['filtro'] : 'proximas';

// 2. Preparamos el query base
$sql = "SELECT c.id_cita, c.fecha_hora, c.motivo, m.nombre AS nombre_mascota, c.estado_atencion
        FROM cita_medica c
        INNER JOIN mascota m ON c.id_mascota = m.id_mascota
        WHERE m.id_propietario = ?";

// 3. Añadimos la condición del filtro
$filtro_sql = "";
$order_sql = " ORDER BY c.fecha_hora ASC"; // Por defecto, las próximas primero

if ($filtro == 'pasadas') {
    $filtro_sql = " AND c.fecha_hora < NOW()";
    $order_sql = " ORDER BY c.fecha_hora DESC"; // Las pasadas, de la más reciente a la más antigua
} elseif ($filtro == 'proximas') {
    $filtro_sql = " AND c.fecha_hora >= NOW()";
}
// Si $filtro == 'todos', no se añade filtro de fecha y se usa el $order_sql por defecto

// 4. Combinamos el query
$sql = $sql . $filtro_sql . $order_sql;
    ...
    <main class="page-content">
        <div class="table-container">
            ...
            <div class="filter-buttons">
                <a href="ver_citas.php?filtro=proximas" class="btn-filter <?php if ($filtro == 'proximas') echo 'active'; ?>">Próximas</a>
                <a href="ver_citas.php?filtro=pasadas" class="btn-filter <?php if ($filtro == 'pasadas') echo 'active'; ?>">Pasadas</a>
                <a href="ver_citas.php?filtro=todos" class="btn-filter <?php if ($filtro == 'todos') echo 'active'; ?>">Todas</a>
            </div>
